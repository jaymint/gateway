...
#         self.channel.exchange_declare(exchange='message', passive=True)
#         print("connected")
...
#         """
#         self._channel = None
#         print("disconnectedddddddddddddddddddddddddd")
#         # if self._stopping:
...
class MQ():
    ...
    def connect(self):
        if self.Connected == False:
            self.mqConn = amqp.Connection(host=self.Config.HOST, userid=self.Config.USERNAME, password=self.Config.PASSWORD, virtual_host=self.Config.VHOST, insist=False)
            self.channel = self.mqConn.channel()
            self.channel.exchange_declare(exchange='message',type='direct')
            #self.channel.queue_declare(queue=self.Config.EVENTQUEUENAME, durable=bool(Config.DURABLE))
            #self.channel.queue_declare(queue=self.Config.SNAPSHOTQUEUENAME, durable=bool(Config.DURABLE))
            #self.channel.queue_bind(queue=self.Config.EVENTQUEUENAME,exchange=self.Config.EXCHANGE,routing_key=self.Config.EVENTRKEY)
            #self.channel.queue_bind(queue=self.Config.SNAPSHOTQUEUENAME,exchange=self.Config.EXCHANGE,routing_key=self.Config.SNAPSHOTRKEY)
            self.Connected = True


    def sendMQ(self, rkey,message):
        if self.Connected == True:
            lMessage = amqp.Message(message)
            self.channel.basic_publish(lMessage, exchange=self.Config.EXCHANGE, routing_key=rkey) 